...
/*
cchar* solicitu::GetName() const { return "Solicitus"; }
cchar* solicitu::GetDescription() const { return "god of stress and hopeless situations"; }
int solicitu::GetAlignment() const { return AN; }
int solicitu::GetBasicAlignment() const { return TOPPLED; }
col16 solicitu::GetColor() const { return NEUTRAL_BASIC_COLOR; }
col16 solicitu::GetEliteColor() const { return NEUTRAL_ELITE_COLOR; }
*/
...
void valpurus::PrayGoodEffect()
{
  if(RAND() & 1)
  {
    ADD_MESSAGE("You hear booming voice: \"THESE WILL PROTECT THEE FROM MORTIFER, DEFEAT HIM, MY PALADIN!\" A sword and shield glittering with holy might appears from nothing.");
...
void solicitu::Pray()
{
	ADD_MESSAGE("You hear an anguished voice crying: \"Legifer, forgive me! My trangression was aeons ago!\"", GetName());
	//randomise
	ADD_MESSAGE("You hear an anguished voice crying: \"I am powerless to help you mortal!\"", GetName());
	//this pray response should probably be a function of story state
}

void solicitu::PrayGoodEffect()
{
  //ADD_MESSAGE("Suddenly, the fabric of space experiences an unnaturally powerful quantum displacement! You teleport away!");
  //PLAYER->Move(game::GetCurrentLevel()->GetRandomSquare(PLAYER), true);
}

void solicitu::PrayBadEffect()
{
  //ADD_MESSAGE("Suddenly, the fabric of space experiences an unnaturally powerful quantum displacement!");
  //PLAYER->TeleportSomePartsAway(1 + (RAND() & 1));
  //PLAYER->CheckDeath(CONST_S("shattered to pieces by the wrath of ") + GetName(), 0);
