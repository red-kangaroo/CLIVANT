...
/* //ghastly piece of code that does not work
olterrain* level::FindWard(const room* Room) const
{
  std::vector<olterrain*> Found;
  olterrain* OLTerrain;
  ...
  for(int x = RoomPos.X; x < (RoomPos.X + RoomSize.X); ++x)
    ...
    {
      OLTerrain = GetLSquare(x,y)->GetOLTerrain();
      if(OLTerrain && OLTerrain->IsWard())
				return OLTerrain;//Found.push_back(OLTerrain);
    }
