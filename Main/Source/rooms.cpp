...
truth bananadroparea::DropItem(character* Dropper, item* Item, int)
{
	...
	{
		if(game::TruthQuestion(CONST_S("Do you wish to plant the mango seedling at this time? [y/N]")) && game::TweraifIsFree() && (game::GetTeam(NEW_ATTNAM_TEAM)->GetRelation(Dropper->GetTeam()) != HOSTILE))
		{
			game::TextScreen(CONST_S(	"You plant the seedling of the Holy Mango Tree in New Attnam.\n"
...
{
  if(game::GetStoryState() == 2 || game::GetTeam(KHARAZ_ARAD_TEAM)->GetRelation(Visitor->GetTeam()) == HOSTILE)
    return true;
...
truth vault::DropItem(character* Visitor, item* Item, int)
{
  ...
  if(Visitor->IsPlayer())
  {
    ...
    if(game::TruthQuestion(CONST_S("Do you wish to donate this item to the Vault? [y/N]")))
      return true;
  }
...
{
  if(game::GetStoryState() == 2 || game::GetTeam(KHARAZ_ARAD_TEAM)->GetRelation(HungryMan->GetTeam()) == HOSTILE)
    return true;
...
{
  if(game::GetStoryState() == 2 || !Teleporter || game::GetTeam(KHARAZ_ARAD_TEAM)->GetRelation(Teleporter->GetTeam()) == HOSTILE)
    return;
...
{
  if(game::GetStoryState() != 2 && Guilty)
    Guilty->GetTeam()->Hostility(game::GetTeam(KHARAZ_ARAD_TEAM));
}
