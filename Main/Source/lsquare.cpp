...
truth lsquare::Parasitize(const beamdata& Beam)
{
  ...
  if(Dude)
  {
		if(Dude->GetTorso()->CanHaveParasite())
		{
			if(Beam.Owner)
				Beam.Owner->Hostility(Dude);
...
truth lsquare::SoftenMaterial(const beamdata& Beam)
{
  ...
  {
    if(Char->IsPlayer())
      ADD_MESSAGE("Your %s glows yellow for a moment!", RandomItem->CHAR_NAME(UNARTICLED));
	...
	if(NewMaterial != NONE) /* Don't Forget! It is an ugly thing, I know, but removal = seg-fault since cannot have NONE material */
		{
			RandomItem->ChangeMainMaterial(MAKE_MATERIAL(NewMaterial)); /*->SpawnMore()*/
			if(OldMaterial->GetConfig() != NewMaterial)
				Changed = 1;
		}
