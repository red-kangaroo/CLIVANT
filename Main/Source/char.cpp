...
truth character::HasCurdledBlood() const
{
  for(stackiterator i = GetStack()->GetBottom(); i.HasItem(); ++i)
    if(i->IsKleinBottle())
			if(i->GetSecondaryMaterial())
			if(i->GetSecondaryMaterial()->GetConfig() == CURDLED_OMMEL_BLOOD)
				return true;
	...
	for(int c = 0; c < GetEquipments(); ++c)
  {
    ...
    if(Item && Item->IsKleinBottle())
			if(Item->GetSecondaryMaterial())
				if(Item->GetSecondaryMaterial()->GetConfig() == CURDLED_OMMEL_BLOOD)
					return true;
  }
...
truth character::HasOmmelBlood() const
{
  for(stackiterator i = GetStack()->GetBottom(); i.HasItem(); ++i)
    if(i->IsKleinBottle())
			if(i->GetSecondaryMaterial())
				if(i->GetSecondaryMaterial()->GetConfig() == OMMEL_BLOOD)
					return true;
	...
	for(int c = 0; c < GetEquipments(); ++c)
  {
    ...
    if(Item && Item->IsKleinBottle())
			if(Item->GetSecondaryMaterial())
				if(Item->GetSecondaryMaterial()->GetConfig() == OMMEL_BLOOD)
					return true;
  }

	//item* KB = Arm->GetWielded();
////
 //   if(KB->IsKleinBottle())
	//		if(KB->GetSecondaryMaterial())
	//		if(KB->GetSecondaryMaterial()->GetConfig() == OMMEL_BLOOD)
				return true;
...
truth character::CurdleOmmelBlood() const
{
	...
	for(int c = 0; c < GetEquipments(); ++c)
  {
	...
	//		if(KB->GetSecondaryMaterial()->GetConfig() == OMMEL_BLOOD)
	//		{
	//			KB->ChangeSecondaryMaterial(MAKE_MATERIAL(CURDLED_OMMEL_BLOOD));
	//			return true;
	//		}
...
truth character::RemoveCurdledOmmelBlood()
{
  ...
  for(int c = 0; c < GetEquipments(); ++c)
  {
	...
	//item* KB = Arm->GetWielded();
	//if(KB && KB->IsKleinBottle())
	//	if(KB->GetSecondaryMaterial())
	//		if(KB->GetSecondaryMaterial()->GetConfig() == CURDLED_OMMEL_BLOOD)
	//		{
	//			KB->RemoveFromSlot();
	//			KB->SendToHell();
	//			return true;
...
void character::Save(outputfile& SaveFile) const
{
  ...
  SaveFile << TemporaryState << EquipmentState << Money << GoingTo << RegenerationCounter << Route << Illegal << CurrentSweatMaterial;
  ...
  for(uint c = 0; c < ItemVector.size(); ++c)
    if(ItemVector[c]->CanBeSeenBy(this) && ItemVector[c]->IsPickable(this))
    {
      ...
      if(CheckFood && UsesNutrition() && !CheckIfSatiated()
			...
			//if( (ItemVector[c])->IsThrowingWeapon() && IsRangedAttacker() //&& HasTheseCategoriesForThrowing()
			//	&& TryToAddToInventory(ItemVector[c]))
			//	return true;
			//	
			
/* Part of an attempt to generalize the throwing item pick-up routine... Grr ...had to resort to having a specialised algorithm for each character who throws stuff*/
			//if( (//ItemVector[c])->IsThrowingWeapon() 
			//			IsRangedAttacker() 
			//			&& ((ItemVector[c])->GetType() & GetWhatCategoryToThrow() /*& !WEAPON*/) 
			//			&& TryToAddToInventory(ItemVector[c])))
			//		return true;
			
			//if( (//ItemVector[c])->IsThrowingWeapon() 
			//			IsRangedAttacker() 
			//			&& ((ItemVector[c])->GetType() & WEAPON) 
			//			&& ((ItemVector[c])->GetConfig() & GetWhatWeaponConfigToThrow())
			//			&& TryToAddToInventory(ItemVector[c])))
			//	return true;
			
			if((ItemVector[c]->GetThrowItemTypes() & GetWhatThrowItemTypesToThrow()) 
				&& IsRangedAttacker()
				&& TryToAddToInventory(ItemVector[c]))
				return true;
    }
...
void character::TeleportRandomly(truth Intentional)
{
  ...
  if(IsPlayer())
	...
	if(!PossibleRoom) //if it's outside of a room
	{
		if(TelePos != ERROR_V2)
			Move(TelePos, true);
		else
	...
	else if(PossibleRoom && PossibleRoom->IsOKToTeleportInto()) // If it's inside of a room, check whether a ward is active that might impede the player
	{
		if(TelePos != ERROR_V2)
			Move(TelePos, true);
		else
...
void character::LoadDataBaseStats()
{
  ...
  SetMoney(GetDefaultMoney());
	SetInitialSweatMaterial(GetDefaultSweatMaterial());
...
{
  EditExperience(INTELLIGENCE, 750, Amount << 5);
  EditExperience(WISDOM, 750, Amount << 5);
...
//truth character::ChangeSweatMaterial(int NewSweatMaterial) const
//{
//  if(!IsPlayer())
//    return false;
//		
//	if(CurrentSweatMaterial)
//	{
//		EditCurrentSweatMaterial(NewSweatMaterial);
//		return true;
//	}
//	
//	else
...
void character::DrawPanel(truth AnimationDraw) const
{
  ...
  if(game::PlayerIsRunning())
  {
	...
	/* // This was found to come up twice on the panel
	switch(GetTirednessState())
  {
   case FAINTING:
    FONT->Printf(DOUBLE_BUFFER, v2(PanelPosX, PanelPosY++ * 10), RED, "Fainting");
...
{
  if(IsPlayer())
    ADD_MESSAGE("You decide to rely on your intuition from now on.");
}
...
{
	if(IsPlayer()) //the AI can't be asked position questions! So only the player can hav this state really :/ a bit daft of me
	{
  ...
  if(Level == 1 && TempConfig[0]->CreateUndeadConfigurations)
	...
	if(CharacterDataBase->UndeadVersions)
	{
	  ...
	  if(TempConfig[0]->UndeadCopyMaterials)
	  {
	    ...
	    ConfigDataBase->DefaultSweatMaterial = CharacterDataBase->DefaultSweatMaterial;
	  }
...
{
  if(//!Item->AllowEquip() // could be a grenade silly
			!(GetBurdenState() > STRESSED)
     || !CanUseEquipment()
     //|| GetAttribute(WISDOM) >= Item->GetWearWisdomLimit()
     || Item->GetSquaresUnder() != 1)
    return false;
...
// end of try to add to inventory
...
void character::ReceiveHolyMango(long Amount)
{
  Amount <<= 1;
  EditExperience(ARM_STRENGTH, Amount, 1 << 13);
  EditExperience(LEG_STRENGTH, Amount, 1 << 13);
...
void character::RegenerateStamina()
{
  if(GetTirednessState() != UNTIRED)
  {
    ...
    if(Sweats() && TorsoIsAlive() && !RAND_N(30) && !game::IsInWilderness())
    {
      ...
      long Volume = long(.05 * sqrt(GetBodyVolume() * GetAttribute(ENDURANCE) / 10));
...
void character::AddLiquidHorrorConsumeEndMessage() const
{
  if(IsPlayer())
    ...
    ADD_MESSAGE("%s looks as if the melancholy of the world is on %s shoulders!.", CHAR_NAME(DEFINITE), GetPossessivePronoun().CStr());
}
...
{
	if(ToBeChecked->GetThrowItemTypes() & GetWhatThrowItemTypesToThrow())
		return true;
	else
		return false;
}
...
truth character::CheckThrowItemOpportunity()
{
  if(!IsRangedAttacker() || !CanThrow() || !IsHumanoid() || !IsSmall() || !IsEnabled()) // total gum
	...
	/*
	for(int d = 0; d < 8; ++d) // If there are adjacent hostiles, then do not throw, melee instead!
	{
		square* Square = GetNeighbourSquare(d);
	...
	//int DoNotTestThisSquare = 0;
	//int XLevel = GetLevel()->GetXSize();
	//int YLevel = GetLevel()->GetYSize();
	int HostileFound = 0;
	...
	for(int r = 1; r <= RangeMax; ++r) 
	{
		for(int dir = 0; dir < 8; ++dir) 
		{
			...
			//{DoNotTestThisSquare = 1;}
			// These positions need to be ignored if they are outside the play area, otherwise the game crashes
			//if( (TestPos.X < 0) || (TestPos.Y < 0) || (TestPos.X > XLevel) || (TestPos.Y > YLevel) )
			
			
			//if(!DoNotTestThisSquare)
			if(Level->IsValidPos(TestPos))
			{
				//lsquare* TestLSquare = GetNearLSquare(TestPos);
				...
				if((Dude && Dude->IsEnabled() && (GetRelation(Dude) != HOSTILE)
								...
								/*|| !TestLSquare->IsFlyable()*/ ) // I question the merit of this line
				{
								...
								&& (CandidateDirections[dir] != BLOCKED))
				{
					...
					// maybe put a little flag here
					HostileFound = 1;
				}
			}
			//DoNotTestThisSquare = 0;
		}
	}
  ...
  GetStack()->FillItemVector(ItemVector);
	...
	for(uint c = 0; c < ItemVector.size(); ++c)
    if(CheckInventoryForItemToThrow(ItemVector[c]))
		{
      ToBeThrown = ItemVector[c];
			break;
		}
...
{
  if(/*!IsRangedAttacker() || */ !CanZap() || !IsHumanoid() || !IsSmall() || !IsEnabled()) // total gum
    return false;
  ...
  GetStack()->FillItemVector(ItemVector);
	...
	for(uint c = 0; c < ItemVector.size(); ++c)
	if((ItemVector[c]->GetMinCharges() > 0) && ItemVector[c]->GetPrice()) // bald-faced gum solution for choosing zappables that have shots left. MinCharges needs to be replaced. Empty wands have zero price! 
	{
	...
	//ThrowItem(ZapDirection, ToBeZapped);
