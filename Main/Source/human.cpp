  ...
  for(int x = Rect.X1; x <= Rect.X2; ++x)
	...
	if(Char && Char != this)
	{
	  ...
	  msgsystem::LeaveBigMessageMode();
		...
		{
			if(Char->CurdleOmmelBlood())
				ADD_MESSAGE("Your vial of Ommel Blood vibrates and it contents curdles with the scream of the Enner Beast."); 
		}
...
void mysteryman::BeTalkedTo()
{
  ...
  else
  {
    if(!game::GetMondedrPass())
    {
      ...
      {
	game::TextScreen(CONST_S(	"You hand over the mondedr pass,\n"
...
void raven::BeTalkedTo()
{
  ...
  else
  {
    if(!game::GetRingOfThieves())
    {
      ...
      {
	game::TextScreen(CONST_S(	"You hand over the Ring of Thieves, Raven's eyes glow in amazement.\n" 
...
void priest::BeTalkedTo()
{
	...
	if(PLAYER->TemporaryStateIsActivated(VAMPIRISM))
  {
    ...
    if(PLAYER->GetMoney() >= Price)
    {
      ADD_MESSAGE("\"You seem to have an addiction to drinking blood. Well, everyone has right to little secret habits, but if you wish to donate %ldgp to %s, maybe I could pray %s to remove your vampiric urges, just so you don't victimize our besotted youth.\"", Price, GetMasterGod()->GetName(), GetMasterGod()->GetObjectPronoun());
	...
	return;
      ...
      ADD_MESSAGE("\"You seem to be vampiric. I might be able to do something for that but I need %ldgp for the ritual materials first.\"", Price);
  }
...
void kamikazedwarf::CreateInitialEquipment(int SpecialFlags)
{
	...
	else if(IsGrenadier())
	{
		...
		for(int k = 0; k < 3; k++)
		{
			GetStack()->AddItem(gasgrenade::Spawn());
		}
		GetStack()->AddItem(lantern::Spawn());
		GetCWeaponSkill(AXES)->AddHit(GetWSkillHits());
		GetCurrentRightSWeaponSkill()->AddHit(GetWSkillHits());
...
truth kamikazedwarf::Hit(character* Enemy, v2 HitPos, int Direction, int Flags)
{
  if(!IsPlayer())
  {
	...
	ADD_MESSAGE("%s shouts: \"This time I won't fail, O Great %s!\"", CHAR_DESCRIPTION(DEFINITE), GetMasterGod()->GetName());
			else if(IsGrenadier() && RAND() & 1)
	ADD_MESSAGE("%s shouts: \"What the hell, it'll be quick. Here goes nothing!\"", CHAR_DESCRIPTION(DEFINITE), GetMasterGod()->GetName());
      else if(IsRookie() && RAND() & 1)
	ADD_MESSAGE("%s sobs: \"War is hell!\"", CHAR_DESCRIPTION(DEFINITE), GetMasterGod()->GetName());
			else if(RAND() & 1)
...
void shaman::GetAICommand()
{
 ...
 //   return;
  ...
  if(NearestEnemy && !StateIsActivated(CONFUSED) && !(RAND() % 3))
  {
    ...
    if(CanBeSeenByPlayer())
      ADD_MESSAGE("%s invokes a spell!", CHAR_NAME(DEFINITE));
		...
		switch(RAND() % 33)
		{
		 case 0:
		 case 1:
		 case 2: Square->DrawParticles(RED); Square->Parasitize(Beam); break;
		 case 3:
		 case 4:
		 case 5:
		 ...
		 case 8:
		 case 9:
		 case 10: Square->DrawParticles(RED); Square->InstillPanic(Beam); break;
		 default: break;
		}
	...
	if(!StateIsActivated(CONFUSED) && CheckThrowItemOpportunity())
		return;
...
void warlock::GetAICommand()
{
  ...
  if(NearestEnemy && (NearestEnemyDistance < 10 || StateIsActivated(PANIC)) && RAND() & 3)
  {
	...
	if(!StateIsActivated(CONFUSED) && CheckAIZapOpportunity())
		return;
...
void uldra::GetAICommand()
{
  ...
  if(NearestEnemy && NearestEnemy->GetPos().IsAdjacent(Pos))
  {
    ...
    {
      if(CanBeSeenByPlayer())
	ADD_MESSAGE("%s grins at you and disappears.", CHAR_NAME(DEFINITE));
	...
	if(NearestEnemy)
  {
    ...
    switch(RAND() % 50)
    {
			...
			case 2:
			{
				...
				if(CanBeSeenByPlayer())
				ADD_MESSAGE("%s grins and invokes a spell!", CHAR_NAME(DEFINITE));
				...
				else
				{
					...
					Buffalo->PutTo(Where);
					if(Buffalo->CanBeSeenByPlayer())
						ADD_MESSAGE("Suddenly %s materializes!", Buffalo->CHAR_NAME(INDEFINITE));
			...
			case 3:
			{
				...
				else
				{
					...
					if(Twoheadedmoose->CanBeSeenByPlayer())
						ADD_MESSAGE("Suddenly %s materializes!", Twoheadedmoose->CHAR_NAME(INDEFINITE));
			...
			{	
				...
				else
				{
					...
					Bear->PutTo(Where);
					if(Bear->CanBeSeenByPlayer())
						ADD_MESSAGE("Suddenly %s materializes!", Bear->CHAR_NAME(INDEFINITE));
...
void kabouter::GetAICommand()
{
	...
	if(NearestEnemy)
  {
    ...
    if(CanBeSeenByPlayer() && NearestEnemy->CanHear())
      ADD_MESSAGE("%s whispers sweet nothings into the ear of %s", CHAR_NAME(DEFINITE), NearestEnemy->CHAR_DESCRIPTION(DEFINITE));
		
		if(NearestEnemy->CanTameWithLyre(this) && NearestEnemy->CanHear())
			
	if((LastHealed || game::GetTick() - LastHealed > 10000) && AttachBodyPartsOfFriendsNear())
    return;
		*/
	
	if((/*LastTamed &&*/ game::GetTick() - LastTamed < 100) && NearestEnemy && (NearestEnemyDistance < 2)) // how about LastTamed < 500
  {
  ...
  else if(NearestEnemy)
  {
    ...
    for(int d = 0; d < GetNeighbourSquares(); ++d)
    {
      ...
      if(Square)
      {
				...
				if(Char)
				{
					if(!Char->IsPlayer() /*&& (GetRelation(Char) == HOSTILE)*/) //check the creature's hostility at this point
					{
						if(CanBeSeenByPlayer())
						{
							if(PLAYER->CanHear())
								...
								ADD_MESSAGE("You see %s mouthing mesmerizing spells.", CHAR_NAME(DEFINITE));
						}
						if(Char->CanHear())
						{
							if(Char->CanTameWithLyre(this))
							{
								//if(Char->GetTeam() == GetTeam() && Char->CanBeSeenByPlayer())
									//ADD_MESSAGE("%s seems to be very happy.", Char->CHAR_NAME(DEFINITE));
								/*else */if(Char->GetRelation(this) == HOSTILE && Char->CanBeSeenByPlayer())
								{
									ADD_MESSAGE("%s stops fighting.", Char->CHAR_NAME(DEFINITE));
									if(Char->CanBeSeenByPlayer())
										ADD_MESSAGE("%s seems to be very friendly towards %s.", Char->CHAR_NAME(DEFINITE), CHAR_NAME(DEFINITE));
								}
...
truth vampire::SpecialBiteEffect(character* Char, v2 HitPos, int BodyPartIndex, int Direction, truth BlockedByArmour)
{
  ...
  {
		if(Char->IsHumanoid())
			Char->BeginTemporaryState(VAMPIRISM, 1500 + RAND_N(2000)); // Randomly instigate vampirism
  ...
  if(StateIsActivated(VAMPIRISM))
	{
		...
		{
			if(Char->IsHumanoid())
				Char->BeginTemporaryState(VAMPIRISM, 1000 + RAND_N(500)); // Randomly instigate vampirism
			if(Char->IsPlayer() || IsPlayer() || Char->CanBeSeenByPlayer() || CanBeSeenByPlayer())
				ADD_MESSAGE("%s drains some precious lifeblood from %s!", CHAR_DESCRIPTION(DEFINITE), Char->CHAR_DESCRIPTION(DEFINITE));
...
void regii::BeTalkedTo()
{
	...
	if(!game::GetRegiiTalkState())
		{
			game::TextScreen(CONST_S("\"I am Regii, of the Unite Tweraif Forest Army, or UTFA for short. And you are?\"\n\n"
			...
			game::TextScreen(CONST_S("\"Now the mission is to free the prisoners, but, our intelligence suspects \n"
...
void archangel::CreateInitialEquipment(int SpecialFlags)
{
  ...
  {
   case GOOD:
    Weapon = flamingsword::Spawn(LONG_SWORD, SpecialFlags|NO_MATERIALS);
...
void doctor::BeTalkedTo()
{
  ...
  for(int c = 0; c < PLAYER->GetBodyParts(); ++c)
    if(!PLAYER->GetBodyPart(c) && PLAYER->CanCreateBodyPart(c))
    {
      ...
      for(std::list<ulong>::const_iterator i = PLAYER->GetOriginalBodyPartID(c).begin(); i != PLAYER->GetOriginalBodyPartID(c).end(); ++i)
      {
				...
				{
					HasOld = true;
					long Price = (2*GetAttribute(CHARISMA)) < PLAYER->GetAttribute(CHARISMA) ? 5 : (2*GetAttribute(CHARISMA) - PLAYER->GetAttribute(CHARISMA));
					...
					if(PLAYER->GetMoney() >= Price)
					{
						...
						{
							ADD_MESSAGE("\"I could put your old %s back in exchange for %ld gold pieces, yes yes.\"", PLAYER->GetBodyPartName(c).CStr(), Price);
						...
						ADD_MESSAGE("\"Oh my! Your %s severed! Help yourself and get %ldgp and Party help you too.\"", PLAYER->GetBodyPartName(c).CStr(), Price);
				}
      }
  ...
  if(PLAYER->TemporaryStateIsActivated(POISONED))
  {
    long Price = GetAttribute(CHARISMA) < PLAYER->GetAttribute(CHARISMA) ? 5 : (GetAttribute(CHARISMA) - PLAYER->GetAttribute(CHARISMA));

    if(PLAYER->GetMoney() >= Price)
    {
      ADD_MESSAGE("\"You seem to be rather ill, yes yes. I give you small dose of antidote for %ld gold pieces.\"", Price);
      ...
      ADD_MESSAGE("\"You seem to be rather ill. Get %ld gold pieces and I fix that.\"", Price);
  }
  ...
  if(PLAYER->TemporaryStateIsActivated(LEPROSY))
  {
    ...
    if(PLAYER->GetMoney() >= Price)
    {
      ADD_MESSAGE("\"You seem to have contracted vile disease of leprosy, yes yes. I can give you small dose of medicince for %ld gold pieces.\"", Price);
      ...
      ADD_MESSAGE("\"You seem to be falling apart. Get %ld gold pieces and I fix that.\"", Price);
  }
  ...
  if(PLAYER->TemporaryStateIsActivated(LYCANTHROPY))
  {
    ...
    if(PLAYER->GetMoney() >= Price)
    {
      ADD_MESSAGE("\"You seem to be turning into a werewolf quite frequently. Party must crush epidemic, special priority! If you wish to donate %ldgp to %s, I can remove the canine blood from your veins with acupucnture, yes yes.\"", Price);
	...
	return;
      ...
      ADD_MESSAGE("\"You seem to be lycanthropic. I might be able to do something for that but I need %ldgp for the Party first.\"", Price);
  }
	...
	if(PLAYER->TemporaryStateIsActivated(VAMPIRISM))
  {
    ...
    if(PLAYER->GetMoney() >= Price)
    {
      ADD_MESSAGE("\"You seem to have addiction to drinking blood. Party must crush epidemic, special priority! If you wish to donate %ldgp to %s, I can remove vampiric urges with acupucnture, yes yes.\"", Price);
	...
	return;
      ...
      ADD_MESSAGE("\"You seem to be vampiric. I might be able to do something for that but I need %ldgp for the Party first.\"", Price);
  }
...
void humanoid::SurgicallyDetachBodyPart()
{
  ...
  switch(game::KeyQuestion(CONST_S("What limb? (l)eft arm, (r)ight arm, (L)eft leg, (R)ight leg, (h)ead?"), KEY_ESC, 5, 'l','r','L','R', 'h'))
  {
   ...
   case 'R':
    ...
    break;
   case 'h':
    ToBeDetached = HEAD_INDEX; // maybe no head, should be up to the player to decide.
    break;
   default:
    return;
  }
...
void morbe::CreateCorpse(lsquare* Square)
{
	...
	{
		if(!c)
			Angel = archangel::Spawn(SCABIES);
		else
			Angel = angel::Spawn(SCABIES);
...
void morbe::BeTalkedTo()
{
  ...
  if(PLAYER->HasCurdledBlood())
  {
		if(PLAYER->RemoveCurdledOmmelBlood())
		{
			...
			game::TextScreen(CONST_S( "Morbe begins to relate a dark secret to you from the depths of her knowledge:\n"
	...
	if(game::GetOmmelBloodMission() != 2)
	{
		...
		{
			game::TextScreen(CONST_S("You approach the acolyte standing by the cauldron, she speaks to you.\n\n"
