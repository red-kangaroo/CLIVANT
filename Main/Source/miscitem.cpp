...
void scrolloffireballs::FinishReading(character* Reader)
{
  v2 FireBallPos = ERROR_V2;
	...
	if(Input == ERROR_V2) // esc pressed
	{
		...
		return;
		//Input = Reader->GetPos(); /*That would kill us, precious*/
	}
	...
	else
	{
		...
		/*
		if(Reader->GetNearestEnemy())
			FireBallPos = Reader->GetNearestEnemy()->GetPos();
		else*/
		if(!(RAND() % 3))
		{
...
liquid* kleinbottle::CreateDipLiquid()
{
  return static_cast<liquid*>(GetSecondaryMaterial()->TakeDipVolumeAway());
}
...
void holybook::FinishReading(character* Reader)
{
  if(Reader->IsPlayer())
  {
    ...
    PLAYER->EditExperience(WISDOM, 150, 1 << 12);
		...
		//if(GetMasterGod()->GetConfig() == SOLICITU) // should be IsNotAGod()
  //  {
  //    ADD_MESSAGE("The pages in this holy book of %s are blank.", GetMasterGod()->GetName());
		//	ADD_MESSAGE("It suddenly disappears!");
		//	RemoveFromSlot();
		//	SendToHell();
		//	return;
  //  }
...
truth mangoseedling::Apply(character* Applier)
{
	...
	if((game::GetCurrentDungeonIndex() == NEW_ATTNAM))
	{
		if(Applier->IsPlayer() && game::TweraifIsFree())
		{
			...
			game::TextScreen(CONST_S(	"You plant the seedling of the first new mango tree in New Attnam.\n"
...
/*
alpha christmaslight::GetAlphaD(int Frame) const
{
  Frame &= 31;
  return (Frame * (31 - Frame) >> 3);
}

void christmaslight::CalculateEmitation() const
{
	if(!IsBroken())
    switch(RAND() % 4)
    {
     case 0: Emitation = MakeRGB24(0, 0, 125); break;
     case 1: Emitation = MakeRGB24(0, 125, 0); break;
     case 2: Emitation = MakeRGB24(125, 0, 0); break;
     case 3: Emitation = MakeRGB24(125, 125, 0); break;
    }
	else
...
col16 christmaslight::GetEmitationColor(int Frame) const
{
  if(!IsBroken())
    switch((Frame&127) >> 5)
    {
...
void holymango::AddInventoryEntry(ccharacter* Viewer, festring& Entry, int, truth ShowSpecialInfo) const // never piled
{
  AddName(Entry, INDEFINITE);
...
material* kleinbottle::GetConsumeMaterial(ccharacter* Consumer, materialpredicate Predicate) const
{
  if(SecondaryMaterial
     && (SecondaryMaterial->*Predicate)()
     && Consumer->CanConsume(SecondaryMaterial)
		 && Consumer->StateIsActivated(TELEPORT))
    return SecondaryMaterial;
  else
    return item::GetConsumeMaterial(Consumer, Predicate);
}
...
void pantheonbook::FinishReading(character* Reader)
{
  if(Reader->IsPlayer())
  {
    PLAYER->EditExperience(INTELLIGENCE, 2000, 1 << 12);
    PLAYER->EditExperience(WISDOM, 2000, 1 << 12);
...
void constitution::FinishReading(character* Reader)
{
  if(Reader->IsPlayer())
  {
    game::TextScreen(CONST_S( "You see here an article on repelling an imperialist invasion.\n"
...
/*truth theredlantern::Zap(character* Zapper, v2, int Direction)
{
  if(!IsBroken)
  {
    ADD_MESSAGE("The red lantern discharges a powerful stream of lightning!", CHAR_NAME(DEFINITE));
    Zapper->EditExperience(PERCEPTION, 150, 1 << 10);
