...
void solicitus::BeTalkedTo()
{
  ...
  if(PLAYER->StateIsActivated(PANIC) && !game::PlayerIsSolicitusChampion())
  {
    ...
    {
      game::TextScreen(CONST_S( "Solicitus speaks:\n"
...
col16 noxiousorchid::GetTorsoSpecialColor() const // the flower
{
  if(!GetConfig())
    return MakeRGB16(125 + RAND() % 100, RAND() % 125, RAND() % 100);
  else if(GetConfig() == GREATER)
...
/* All about the spillfluid for the orchid */
		...
		switch(GetConfig())
		{
			...
			//{
				switch(RAND() % 48)
				{
					...
					case 2:
					case 3:
					case 4: Fluid = liquid::Spawn(POISON_LIQUID, 15+RAND()%25); break;
					case 5: Fluid = liquid::Spawn(LIQUID_HORROR, 15+RAND()%25); break;
					case 6:
					case 7:
					case 8: Fluid = liquid::Spawn(SULPHURIC_ACID, 15+RAND()%25); break;
					case 9:
					default: break;
				};
				break;
			...
			//{
				switch(RAND() % 24)
				{
					...
					case 2:
					case 3:
					case 4: Fluid = liquid::Spawn(POISON_LIQUID, 25+RAND()%25); break;
					case 5: Fluid = liquid::Spawn(LIQUID_HORROR, 25+RAND()%25); break;
					case 6:
					case 7:
					case 8: Fluid = liquid::Spawn(SULPHURIC_ACID, 25+RAND()%25); break;
					case 9:
					default: break;
				};
				break;
			...
			//{
				switch(RAND() % 24)
				{
					...
					case 2:
					case 3:
					case 4: Fluid = liquid::Spawn(POISON_LIQUID, 50+RAND()%50); break;
					case 5: Fluid = liquid::Spawn(LIQUID_HORROR, 50+RAND()%50); break;
					case 6:
					case 7:
					case 8: Fluid = liquid::Spawn(SULPHURIC_ACID, 50+RAND()%50); break;
...
truth fruitbat::IsRetreating() const
{
  ...
  for(stackiterator i = GetStack()->GetBottom(); i.HasItem(); ++i)
    if( (*i)/*->GetMainMaterial()->IsFruit()*/->IsFood() )
      return true;
...
if(RandomValue < 250)
		  NewPlant = mushroom::Spawn();
		else if(RandomValue < 1500)
		  NewPlant = magicmushroom::Spawn();
		else
		  NewPlant = magicmushroom::Spawn();
...
{
  for(int c = 0; c < 1; ++c)
    Square->AddItem(wand::Spawn(WAND_OF_CLONING));
...
if(RandomValue < 250)
		  ...
		  NewPlant = carnivorousplant::Spawn(LILY);
		else
		  NewPlant = carnivorousplant::Spawn(SHAMBLING);
     ...
     && (CradleSquare->GetWalkability() & WALK))
  {
    ...
    if((SpoiledItems && MushroomsNear < 1 && !RAND_N(2)) || (MushroomsNear < 3 && !RAND_N((1 + MushroomsNear) * 2)))
    {
      ...
      if(Child->CanBeSeenByPlayer())
	ADD_MESSAGE("%s sprouts from the ground.", Child->CHAR_NAME(INDEFINITE));
    }
  }
