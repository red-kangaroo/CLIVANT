...
struct characterdatabase : public databasebase
{
  ...
  int AttackStyle;
  long ClassStates;
	long WhatThrowItemTypesToThrow;
...
class character : public entity, public id
{
 public:
  ...
  void EditMoney(long What) { Money += What; }
	truth ChangeSweatMaterial(int) const;
  ...
  DATA_BASE_VALUE(long, ClassStates);
	DATA_BASE_VALUE(long, WhatThrowItemTypesToThrow)
  ...
  DATA_BASE_VALUE(int, DefaultSweatMaterial);
  ...
  void Slow();
	...
	truth CheckInventoryForItemToThrow(item*);
	//virtual truth HasRangedAttackMode() const { return IsUsingArms(); }
