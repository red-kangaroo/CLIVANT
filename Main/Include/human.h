...
{
 public:
  petrus(); // there is a petrus
  virtual ~petrus(); // if there is no petrus, then librarian's betalkedto replies change
  virtual void Save(outputfile&) const;
  virtual void Load(inputfile&);
  virtual void BeTalkedTo();
...
//probably do this, and override the torso main colours etc
...
CHARACTER(mangogrower, humanoid)
{
};
